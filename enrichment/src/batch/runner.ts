...
export async function runBatch(
  ...
  const startingCheckpoint = toCheckpointState(
    checkpoint,
  ...
  writeProductCSV(
    orderedEntries.map((entry) => entry.row),
    options.outputPath,
  );
...
async function enrichProduct(
  ...
  try {
    const images = await prepareProductImages(
      product.sku,
      manifestPath,
      imagesDir,
    );
...
function updateCheckpointState(
  ...
  const completedBySku = new Map(
    checkpointState.completed.map(
      (entry) => [entry.sku, entry.status] as const,
    ),
  );
