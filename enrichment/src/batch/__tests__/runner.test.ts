...
function createResult(partial?: Partial<EnrichmentResult>): EnrichmentResult {
  return {
...
describe("runBatch", () => {
  ...
  it("processes all products and returns results array", async () => {
    ...
    expect(result.results.map((entry) => entry.sku)).toEqual([
      "SKU-1",
      "SKU-2",
    ]);
    ...
    const adapter: EnrichmentAdapter = {
      ...
      enrich: vi
        ...
        .mockResolvedValueOnce(
          createResult({ status: "success", fillRate: 1 }),
        ),
    };
